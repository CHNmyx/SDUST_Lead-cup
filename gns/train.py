...
flags.DEFINE_enum(
    'aggregate_mode', 'attention', ['attention', 'add', 'mean','max'],
    help='Aggreagation manner of graph neural network')
flags.DEFINE_integer('batch_size', 8, help='The batch size.')
...
def predict(device: str):
  ...
  simulator = _get_simulator(metadata, FLAGS.noise_std, FLAGS.noise_std, FLAGS.aggregate_mode, device)
...
def acceleration_loss(pred_acc, target_acc, non_kinematic_mask):
  ...
  Args:
    ...
    target_acc: Target accelerations.flags["aggregate_mode"]
...
def train(rank, flags, world_size, device):
  ...
  if device == torch.device("cuda"):
    serial_simulator = _get_simulator(metadata, flags["noise_std"], flags["noise_std"],flags["aggregate_mode"], rank)
