...
def edge_timestamp_feat(edge_timestamp, edge_index, x):
    ...
    # 将 edge_timestamp_sum 转换为 NumPy 数组
    edge_timestamp_sum = np.asarray(edge_timestamp_sum).reshape(-1)  # 转换为一维数组
    ...
    return (
        edge_timestamp_mean,
        preprocessing.normalize(edge_timestamp_sum.reshape(1, -1)).flatten(),  # 归一化并转为一维数组
...
def read_dgraphfin(folder):
    ...
    # 将 edge_timestamp_sum 转换为二维数组，便于与 x 拼接
    edge_timestamp_sum = edge_timestamp_sum.reshape(-1, 1)  # 转换为列向量
